...
(function (d3, undefined) {
    //helpers
    function applyTicks(_axis, ticks) {
        //helper for applying ticks to axis or grid
    ...
    Polymer({
         ...
         */
        prepare: function prepare(width, height, target) {
            var me = this,
                xAxis = me.axes[0],
                yAxis = me.axes[1],
                y1Axis = me.axes[2],
            ...
            if (dataPresent) {
                me.metadata.forEach(function (dataset) {
                    ...
                    var getterX,
                        getterY,
                        axis = me.axes.filter(function (axis) {
            ...
            var axisOrientation,
                orientations = ['bottom', 'left', 'right'];
            ...
            svg.append('defs').append('clipPath').attr('id', 'cp' + uid).append('rect').attr('width', width)
            //height & translate manipulation for proper display
            .attr('height', height + 2).attr('transform', 'translate(0 ,-2)');
            ...
            if (!external) {
                ...
                //if external apply styles as attributes
                if (!me.factory) {
                    me.prepare();
                }
            }
            ...
            svg.selectAll('.line').attr('stroke', function (d, key) {
                return me.colors[key];
            });
        ...
        _sizeChanged: function _sizeChanged() {
            this.sizeChanged = true;
            this.prepare();
        },

        //checker for redraw
        _checkAndRedraw: function _checkAndRedraw() {
            if (this.prepared) {
                this.redraw();
            } else {
                this.prepare();
        ...
        //helpers
        getXgetter: function getXgetter(dataset) {
            var me = this,
                sample;
        ...
        getYgetter: function getYgetter(dataset) {
            var me = this,
                sample;
            if (!me.data || !me.data.length) {
                throw Error('Can\'t sample records, no data');
